...
const SetpsLevel = (props) => {
  return (
    <PageWrapper navBarText="Steps 步骤条 水平方向的步骤条" {...props}>
      <WingBlank mode={20} className={styles["stepsExample"]}>
        <div className={styles["sub-title"]}>Horizontal small size</div>
        <WhiteSpace />
        <Steps current={1} direction="horizontal" size="small">
          {steps}
        </Steps>
        <div className={styles["sub-title"]}>Horizontal default size</div>
        <WhiteSpace />
        <Steps current={1} direction="horizontal">
          {steps}
        </Steps>
        <div className={styles["sub-title"]}>Horizontal customized icon</div>
        <WhiteSpace />
